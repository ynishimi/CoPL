...
class DBClosure:
    ...
    def __str__(self):
        env_str = ", ".join(str(v).lower() if isinstance(v, bool) else str(v) for v in self.val_env)
        return f"({env_str})[{self.func_body}]"
Value = Union[int, bool, DBClosure]
...
# -------------------------------------------------------------------
# 4. 評価の導出規則 (DB-E-Rules)
# -------------------------------------------------------------------
class DBEDerivation:
    def format(self, i=0) -> str: raise NotImplementedError
def format_val_env(val_env: List[Value]) -> str:
    if not val_env: return ""
    return ", ".join(str(v).lower() if isinstance(v, bool) else str(v) for v in val_env) + " "
@dataclass
class DBEInt(DBEDerivation):
...
@dataclass
class DBEBool(DBEDerivation):
    val_env: List[Value]; value: bool
    def format(self, i=0): return f'{" "*i}{format_val_env(self.val_env)}|- {str(self.value).lower()} evalto {str(self.value).lower()} by E-Bool {{}};'
@dataclass
class DBEVar(DBEDerivation):
    val_env: List[Value]; index: int; value: Value
    def format(self, i=0): return f'{" "*i}{format_val_env(self.val_env)}|- #{self.index} evalto {str(self.value).lower() if isinstance(self.value, bool) else str(self.value)} by E-Var {{}};'
@dataclass
class DBEFun(DBEDerivation):
...
@dataclass
class DBEGeneric(DBEDerivation):
    val_env: List[Value]; expr: DBExpression; value: Value; rule_name: str; premises: List[Union[DBEDerivation, 'BDerivation']]
    def format(self, i=0) -> str:
        indent = " " * i; premise_str = "\n".join(p.format(i + 1) for p in self.premises)
...
# 6. 評価エンジン (dbevaluate)
# -------------------------------------------------------------------
def dbevaluate(val_env: List[Value], db_expr: DBExpression) -> Tuple[Value, DBEDerivation]:
    if isinstance(db_expr, DBIntLiteral): return db_expr.value, DBEInt(val_env, db_expr.value)
    if isinstance(db_expr, DBBoolLiteral): return db_expr.value, DBEBool(val_env, db_expr.value)
    if isinstance(db_expr, DBIndex):
        val = val_env[-(db_expr.index)]
        return val, DBEVar(val_env, db_expr.index, val)
    if isinstance(db_expr, DBBinaryOp):
        v1, p1 = dbevaluate(val_env, db_expr.left)
        v2, p2 = dbevaluate(val_env, db_expr.right)
        ...
        return result, DBEGeneric(val_env, db_expr, result, rule_name, [p1, p2, b_deriv])
    if isinstance(db_expr, DBIf):
        cond_val, p1 = dbevaluate(val_env, db_expr.cond)
        if not isinstance(cond_val, bool): raise TypeError("If condition must be a boolean")
        if cond_val:
            result, p2 = dbevaluate(val_env, db_expr.true_branch)
            return result, DBEGeneric(val_env, db_expr, result, "E-IfT", [p1, p2])
        else:
            result, p2 = dbevaluate(val_env, db_expr.false_branch)
            return result, DBEGeneric(val_env, db_expr, result, "E-IfF", [p1, p2])
    if isinstance(db_expr, DBLet):
        v1, p1 = dbevaluate(val_env, db_expr.bound_expr)
        new_env = val_env + [v1]
        v2, p2 = dbevaluate(new_env, db_expr.body_expr)
        return v2, DBEGeneric(val_env, db_expr, v2, "E-Let", [p1, p2])
    if isinstance(db_expr, DBFun):
        closure = DBClosure(val_env, db_expr)
        return closure, DBEFun(val_env, db_expr, closure)
...
class DBParser:
    ...
    @classmethod
    def from_text(cls, text: str):
        keywords = {'if', 'then', 'else', 'true', 'false', 'let', 'in', 'fun'}
        token_spec = [('ARROW', r'->'), ('DB_INDEX', r'#\d+'), ('KEYWORD', r'\b(if|then|else|true|false|let|in|fun)\b'), ('INT', r'-?\d+'), ('OP', r'[+*<()-.=]'), ('SKIP', r'\s+'), ('MISMATCH', r'.')]
        tok_regex = '|'.join(f'(?P<{pair[0]}>{pair[1]})' for pair in token_spec)
        tokens: List[Token] = []
        for mo in re.finditer(tok_regex, text):
            kind, value = mo.lastgroup, mo.group()
    ...
    def parse_let(self) -> DBExpression:
        self.consume('let'); self.consume('.'); self.consume('=')
        bound_expr = self.parse_expression(); self.consume('in'); body_expr = self.parse_expression()
        return DBLet(bound_expr, body_expr)
    def parse_fun(self) -> DBExpression:
        self.consume('fun'); self.consume('.'); self.consume('->')
        body = self.parse_expression()
        return DBFun(body)
    ...
    def parse_mul(self) -> DBExpression:
        node = self.parse_primary()
        while self.current_token() and self.current_token()[1] == '*': self.consume('*'); right_node = self.parse_primary(); node = DBBinaryOp('*', node, right_node)
        return node
        ...
        for line in sys.stdin:
            ...
            try:
                if "==>" in line: # Translation Task
                    ...
                    _, derivation_tree = dbevaluate(val_env, db_ast)
                    formatted_tree = derivation_tree.format(0)
                
                else:
                    raise SyntaxError("入力には '==>' (変換) または 'evalto' (評価) を含めてください。")
